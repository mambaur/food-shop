...
class Beranda extends CI_Controller{
	function __construct(){
		parent::__construct();		
		$this->load->model('admin/Transaksi_admin');
	...
	public function index(){
		$data['total'] = $this->Transaksi_admin->totalPemasukan();
		...
		$data['pesan'] = $this->Transaksi_admin->tampil_pesan();
	...
	public function cari(){
		...
		if ($cek>=1) {
			...
			$data['user'] = $this->db->get_where("user",  ['level_id_level' => '111'])->num_rows();
			$data['kontak'] = $this->db->get("tentang")->num_rows();
			$data['pesan'] = $this->Transaksi_admin->tampil_pesanid($kodepesan);
			$this->load->view('element/admin/admin-header',$data);
			$this->load->view('admin/admin-beranda',$data);
			$this->load->view('element/admin/admin-footer');
		}else{
	...
	public function pengiriman(){
		...
		$data['kirim'] = $this->Transaksi_admin->tampil_pengiriman($idkirim);
	...
	public function status(){
		...
		$status = 'Terbayar';
		$this->Transaksi_admin->updatestatus($idpesan,$status);
	...
	public function detail(){
		...
		$data['inv'] = $this->Transaksi_admin->invoice($idpesan2,$iduser);
		$data['inv2'] = $this->Transaksi_admin->user($iduser);
